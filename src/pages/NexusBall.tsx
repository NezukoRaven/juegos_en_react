...
interface NexusBallProps {
  onBack: () => void;
}

const NexusBall: React.FC<NexusBallProps> = ({ onBack }) => {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      const updatedBalls = balls.map(ball => {
        ...
        return {
          ...ball,
          x: newX < ball.radius ? ball.radius : newX > (canvas?.width ?? 0) - ball.radius ? (canvas?.width ?? 0) - ball.radius : newX,
          y: newY < ball.radius ? ball.radius : newY > (canvas?.height ?? 0) - ball.radius ? (canvas?.height ?? 0) - ball.radius : newY,
          dx: newDx,
          dy: newDy
        };
      });
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden bg-white">
      ...
      <div className={`absolute top-16 right-4 transition-all duration-300 ease-in-out transform ${showMenu ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0'
        }`}>
        <Card className="p-4 bg-white/80 backdrop-blur-sm shadow-lg rounded-lg">
          <div className="space-y-4">
            <div className='flex items-center space-x-2 beetwen'>
              <Button
                variant="outline"
                size="icon"
                onClick={onBack}
              ...
              <CardTitle>Nexus Ball</CardTitle>
            </div>
            <div>
