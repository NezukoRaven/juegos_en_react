...
interface SpiderBallProps {
  onBack: () => void;
}

const SpiderBall: React.FC<SpiderBallProps> = ({ onBack }) => {
  const [pointerPos, setPointerPos] = useState({ x: 0, y: 0 });
  const [staticPoints, setStaticPoints] = useState<{ x: number; y: number }[]>([]);
      ...
      <div className={`fixed top-0 right-0 h-full w-64 bg-gray-900/90 backdrop-blur-sm text-white p-4 transform transition-transform duration-300 ease-in-out ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="space-y-6 mt-12">
          <div className='flex items-center space-x-2 beetwen'>
            ...
            <CardTitle>Sipider Ball</CardTitle>
          </div>
